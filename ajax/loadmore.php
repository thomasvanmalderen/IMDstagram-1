...
//$statement = $PDO->prepare("SELECT DISTINCT  p_id, picture, description, posts.posttime, username, avatar, users.u_id, location, filter, comments.c_id, comments.comment FROM posts LEFT JOIN users ON users.u_id = posts.idUser LEFT JOIN follows ON follows.idFollowed = posts.idUser LEFT JOIN comments ON comments.idPost = posts.p_id WHERE follows.idFollowing = " . $_SESSION['u_id'] . " OR Posts.idUser = " . $_SESSION['u_id'] . " ORDER BY posts.posttime desc LIMIT $offset, 5");
